function [realtime,runN]=mainCodeF_ET(simulation,fnameI,nodes,P1,V1,CO2i,realInitialT,VBC,atmosphAll,obsNnodes,NSubreg2,NSubreg3,NSubreg4,NSubreg5)
tic
folder=pwd;
j=0;
...
if exist('finalresults.mat','file')==[2],
    ...
    finalresults=resultsInitial(fnameI,obsNnodes);
end
%-------------------------------------------------------------------------
files=dir([fname '*']);
fileN=size(files,1);
...
%Multicomponent 13, regular solutes 10 (or multicomponent with regular exe files), no solutes 6. 
[balanceI(1:13,:),balanceF(1:13,:)]=readBalance_multi([fname '1']);
...
ObsN(1,:)=obsNod([fname '1'],1,obsNnodes);
...
[pHout,calcite,gypsum,equil_pHvbc,phreeqcR,pHnodes,pHsub2,pHsub3,pHsub4,pHsub5]=PHREEQC(volume, V1, [fname '1'], CO2, cupConc,1,nodes,VBC,j,obsNnodes,NSubreg2,NSubreg3,NSubreg4,NSubreg5);
...
for k=2:time
    ...
    %Multicomponent 13, regular solutes 10 (or multicomponent with regular exe files), no solutes 6. 
    [balanceI((k-1)*13+1:k*13,:),balanceF((k-1)*13+1:k*13,:)]=readBalance_multi([fname runN]);
    ...
    ObsN(k,:)=obsNod([fname runN],k,obsNnodes);
    ...
    [pHout(k,:), calcite(k,:),gypsum(k,:),equil_pHvbc(k,:),phreeqcRr,pHnodes(k,:),pHsub2(k,:),pHsub3(k,:),pHsub4(k,:),pHsub5(k,:)]=PHREEQC(volume, V1, [fname runN ], CO2, cupConc,k,nodes,VBC,j,obsNnodes,NSubreg2,NSubreg3,NSubreg4,NSubreg5);
    ...
    if (volume(k)-volume(k-1))/(volume(2)-volume(1))<0.01 || volume(k)>400 
        j=1;
    [pHout(k,:), calcite(k,:),gypsum(k,:),equil_pHvbc(k,:),phreeqcRr,pHnodes(k,:),pHsub2(k,:),pHsub3(k,:),pHsub4(k,:),pHsub5(k,:)]=PHREEQC(volume, V1, [fname runN ], CO2, cupConc,k,nodes,VBC,j,obsNnodes,NSubreg2,NSubreg3,NSubreg4,NSubreg5);
        break
    end
...
data=[realtime', equil_pHvbc, pHout,volume,pressH_BC,CO2',calcite(:,6),gypsum(:,6),pHsub2,pHsub3,pHsub4,pHsub5];
