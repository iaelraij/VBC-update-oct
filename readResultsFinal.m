...
function [seepC,ScupC,time,general_data,ScupCf,timeF,obsNodf,Ca,K,Cl,...
    ...
    obsNchem,ScupChem,SeepQ,Scupmeqcm3,SoluteBal]=readResultsFinal(finalresults,resultsSolLongR)
...
for i=1:size(resultsSolLongR,2)
    ...
    %timeF(i,:)=finalresults(i+1).general_data(end,1);
    obsNodf(i,:)=finalresults(i+1).ObsN(end,:);
    ...
    soilBf_Ca(i,:)=SoluteBal(i).Ca(end,:);
    soilBf_Mg(i,:)=SoluteBal(i).Mg(end,:);
    soilBf_Na(i,:)=SoluteBal(i).Na(end,:);
    soilBf_K(i,:)=SoluteBal(i).K(end,:);
    soilBf_Alk(i,:)=SoluteBal(i).Alk(end,:);
    soilBf_SO4(i,:)=SoluteBal(i).SO4(end,:);
    soilBf_Cl(i,:)=SoluteBal(i).Cl(end,:);
    soilBf_tracer(i,:)=SoluteBal(i).tracer(end,:);
...
for i=2:size(finalresults,2)
    ...
    PHsoil(i-1,:)=[finalresults(i).general_data(k*10,2) finalresults(i).general_data(k*10,10) finalresults(i).general_data(k*10,11) finalresults(i).general_data(k*10,12)];
